...
import ProfilemenuComponent from "@/components/Channel/ProfilemenuComponent.vue";
import { onBeforeMount, ref } from "vue";
import { fetchy } from "../../utils/fetchy";

let channels = ref<Array<Record<string, string>>>([]);
const getChannelbymember = async () => {
  // get all channels if not specify channelname
  let all_channels;
  try {
    all_channels = await fetchy("/api/channel/member", "GET");
  } catch (_) {
    return;
  }
  ...
  <main>
    ...
    <section v-if="loaded && channels.length !== 0">
      <article class="channelmenu" v-for="channel in channels" :key="channel._id">
        <ProfilemenuComponent :channels="channel" />
        <!-- <button class="joinbutton" @click="getexpiretime(channel.name)">check remain time</button>
        <p>the remain time is {{expiretime}} hours</p> -->
        <br />
      </article>
    </section>
    <br />
